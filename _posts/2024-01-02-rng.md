...
What we did is somewhat similar to abstraction in programming languages. For
instance, in TypeScript, we could have introduced an interface for a PRNG and
declare that `squares64` implements this interface. *Beware!* This is where the
analogy ends. Interfaces in mainstream programming languages are purely
syntactic. As soon as your implementation meets the syntactic requirements that
is, the type signatures match, the compiler would consider your implementation
to be compatible with the interface. Our abstraction is not *syntactic* but
*semantic*: We have replaced a concrete PRNG with an abstract PRNG that
encompasses the behavior of all possible PRNGs. To be completely precise here,
I would have to *prove* that `squares64` does indeed generate values in the
interval $[0, \texttt{WORD_BOUND})$. Here I am saying that it is obvious, which
is a hand-waiving argument. Perhaps, I will show how to do that in a different
blog post.

If you have skimmed through the last two paragraphs, I recommend you to read
them again. What we have done with `squares64` is pretty important, even though
it was not complicated. Developing intuition about which parts of the code
should remain unabstracted and which parts of the code should be abstracted is
one of the most important skills in specification writing.
...
Since I want to look at the examples, I constrain the simulator to try exactly
one run (by setting `--max-samples=1`). By running the simulator multiple times
and comparing the values to what I expected, I am getting a bit of trust in my
specification.
...
```quint
  val invariant = (output >= 0) and (output < bound)
```

We will see how to check this invariant later.

**Property (2).** This property is not as easy to write. For starters, similar
...
Is it game over? Not exactly. If we negate Property (2'), then we will see
that we can talk about all futures again:
...
allows us to reason about all executions. Perhaps, it would be even more
obvious, if I was writing these properties in a temporal logic. However, I
would keep them in English to keep things simple (even though less precise).
If you wonder how I came up with Property (2''), I actually took a formal
negation of a logic formula.

Let's fix $bound$ to be `0xff0001` and $i$ to be `0xff0000` and write the
...
with [Apalache][] to demonstrate that my PRNG code was flawed.

It only remains to see how to check Properties (1)-(2).
