...
choice between the enabled actions. If you know TLA+, `any {
inc(2), dec(3) }` would look like this in TLA+:
...
above nondet-binding could be written as follows in TLA+:
...
specialized domain-specific languages for writing pre- and postconditions.
...
questions:
 ...
 worried about the code logic being broken, or do we really have to figure
...
Since my goal is to understand, whether the generator logic is broken, or not,
my answers to these questions are as follows:

 1. The specification should follow the code logic. Whether the generator's
 distribution is uniform is less important to me at the moment.

 1. We do not care about the exact behavior of `square64`. However, it is
 important that `square64` generates integers in the interval $[0, 2^64)$.
 This means that we want to replace `square64` with an abstraction. You will
 see how we do it below. Since the variable `state` only interacts with
 `square64`, we can simply drop this variable.
