...
Property (1) is a *state invariant*: It should hold in every state.
We can easily write it like follows:
...
Property (2) is not as easy to write. To start with, similar to
$\tla{}$, Quint does not support probabilistic properties out of the box.
Luckily, we only require the probability to be different from zero. Hence,
we can informally rewrite Property (2) as follows:

  "For every integer $i$ from the interval $[0, bound)$, there exists a
  future state in which $output = i$ holds true."
