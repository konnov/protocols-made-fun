...
Having `getOrElse`, we rewrite `send` as you can see below. To save you time,
...
The modified version is also not really bad, though it has a bit of additional
...
implementation details. The good thing is that if you cannot stop thinking
about these `getOrElse` and `put`, you can use them and remove them later, as I
did.
...
When it comes to $\tla{}$, something interesting is happening there:
 ...
 - It possible to construct an empty map (a $\tla{}$ function) but it is not
   entirely obvious how to do it. One way is to write something like $[x \in
