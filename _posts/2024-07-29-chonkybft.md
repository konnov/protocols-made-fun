...
The goal of the validators is to agree on the next block to commit onto a
...
involved only in a few cases, e.g., when a validator receives a proposal, or
when it receives a block from the gossip layer.

The correct validators are progressing in views, starting with view 0. In every
view, a validator may receive a proposal from the view proposer
(programmatically known to all validators), commit a block, issue a timeout, or
switch to the next view, as soon as it has received a justification to do so
(e.g., a timeout quorum certificate). When a validator receives a quorum of
commit messages for a certain block &mdash; signed commit votes from $n - f$
validators &mdash; it commits a block. In this case, the validator also sends
the commit quorum certificate, so late validators could catch up fast, instead
...
is optimized for the case of re-proposing the same block, when the validators
have received sufficiently many commits from a sub-quorum of $n - 3f$
validators.  To this end, each validator stores the summary of the other
validator’s states that it has learned about by receiving messages, e.g., the
...
[Figure 2](#fig2) demonstrates a distributed computation of six validators, with
validator n5 being faulty. Initially, validator n1 sends the proposal for the
block `“val_b1”` to be committed as the block number 0. The validators $n_0$,
$n_1$, $n_2$, and $n_4$ receive this proposal and send their commit vote. The
faulty validator $n_5$ assembles the signed votes of $n_0$, $n_1$, $n_2$, and
...
$n_5$ could send a commit vote of its own. Finally, validator $n_4$ receives the
...
is, reaching a global state, where a correct validator commits one or more
blocks.
...
the effects of Byzantine validators in our specification. It’s often said that
Byzantine validators may exhibit arbitrary behavior. Formal specification
...
We formalize a single step of the faulty validators in the action called
...
In the above code, an arbitrary subset of the Byzantine validators inject their
...
the correct validators are committing three blocks. If you look at the code of
...
case of 4 correct validators and 2 faulty validators ($n=6,f=1,b=2$), even though
